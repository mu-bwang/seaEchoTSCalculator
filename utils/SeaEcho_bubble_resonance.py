...
def resonance_freq(f, c, water, bubble):
    ...
    """
    # Convert all inputs to high precision floats (use float64 for compatibility)
    f = np.float64(f)
    c = np.float64(c)
    omega = np.float64(2 * np.pi * f * 1000)  # Radians/sec
    a = np.float64(bubble.d / 2)  # Bubble radius (m)

    # Harmonic breathing frequency
    f_b = np.float64(1 / (2 * np.pi * a)) * np.sqrt(
        np.float64(3) * bubble.gamma * water.P / water.rho
    )

    # Correction factors
    tau = np.float64(water.sigma * 1e3)  # Surface tension (dyne/cm)
    P_dyn = np.float64(water.P * 10)  # Pressure (dyne/cm^2)
    rho_g = np.float64(bubble.rho_0 * 1e-3)  # Gas density (g/cm^3)
    Cp_g = np.float64(bubble.Cp * 0.2388)  # Specific heat (cal/g/°C)
    K_g = np.float64(bubble.K_th * 0.00239)  # Thermal conductivity (cal/cm/s/°C)

    # Compute X with full precision
    X = np.float64(a * 1e2 * np.sqrt(2 * omega * rho_g * Cp_g / K_g))

    # Corrections for surface tension and thermal conductivity
    d_over_b = np.float64(3 * (bubble.gamma - 1)) * (
        (X * (np.sinh(X) + np.sin(X)) - 2 * (np.cosh(X) - np.cos(X)))
        / (X**2 * (np.cosh(X) - np.cos(X)) + 3 * (bubble.gamma - 1) * X * (np.sinh(X) - np.sin(X)))
    )

    b = np.float64(1 / (1 + d_over_b**2))
    beta = np.float64(1 + 2 * tau / (P_dyn * a * 1e2) * (1 - 1 / (3 * bubble.gamma * b)))

    # Corrected resonance frequency
    f_R = f_b * np.sqrt(b * beta)
    correction_params = np.array([b, d_over_b, beta], dtype=np.float64)
